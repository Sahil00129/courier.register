...
@section('title', "Unknown Invoice List")
@section('content')
    ...
    <div class="layout-px-spacing" id="divbox">
        <div class="page-header">
            <nav class="breadcrumb-one" aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="javascript:void(0);">Unknown Invoice List</a>
                    </li>
                </ol>
            </nav>
        </div>
            ...
            <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                        ...
                        @else
                            @foreach ($tercouriers as $key => $tercourier)
                                <tr>
                                    ...
                                    <?php
                                    if ($tercourier->employee_id == "unknown_code") {
                                        $status = 'Missing Info';
                                        $class = 'btn-danger';
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="partialpaidModalLabel"> UNID ID: @{{ter_id}}</h5>
                                    ...
                                    <form>
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control"
                                                   v-on:change="get_sender_data(po_all_info)"
                                                   v-model="po_all_info" list="sender_data"/>
                                            <datalist class="select2-selection__rendered" id="sender_data">
                                                <option v-for="po_all_info in all_po_data"
                                                        :key="po_all_info.po_number">
                                                    @{{po_all_info.po_number}} : @{{po_all_info.vendor_code}} :
                                                    @{{po_all_info.vendor_name}} : @{{po_all_info.po_value}}  
                                                </option>
                                            </datalist>
                                            <!-- <select
                                          class="form-control"
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-primary" @click="update_po_details()"
                ...
                <div class="modal fade show" id="exampleModal" v-if="cancel_ter_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Cancel UNID</h5>
        ...
        new Vue({
            ...
            cancel_invoice: function() {
                if (this.cancel_remarks != "") {
                    axios.post('/cancel_invoice', {
                            'unid': this.ter_id,
                            'cancel_remarks': this.cancel_remarks
                        })
                        .then(response => {
                            if (response.data) {
                                swal('success', "UNID :" + this.ter_id + " has been cancelled", 'success')
                                location.reload();
                            } else {
                ...
                fetch_po_list: function () {
                    this.all_data = {};
                    //  alert(this.unique_id);

                    axios.post('/fetch_po_list', {
                        'id': this.ter_id
                    })
                        .then(response => {
                            ...
                            this.all_po_data = response.data.all_po_data;
                        }).catch(error => {


                    })
                },
                update_po_details: function () {
                    if (this.ter_remarks != "" && this.po_all_info != "") {
                        var po_number;
                        const sender_data_split = this.po_all_info.split(" : ");
                        po_number = sender_data_split[0];
                        ...
                        axios.post('/update_po_details', {
                            ...
                            .then(response => {
                                console.log(response.data)
                                if(response.data[0] == "po_value_error")
                                {
                                    swal('error', "Total Amount "+response.data[1]+" is Exceeding the PO Value "+response.data[2], 'error')
                                    return 1;

                                }
                                if (!response.data) {
                                    swal('error', "issue with PO", 'error')
                                    // location.reload();
                                }
                                if (response.data) {
                                    swal('success', "UNId :" + this.ter_id + " has been successfully updated", 'success')
                                    location.reload();
                                }
