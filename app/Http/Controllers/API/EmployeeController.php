...
class EmployeeController extends Controller
{
    ...
    {
        try {
            $query = Sender::where('employee_id', $emp_id)->select('id', 'name', 'employee_id', 'grade', 'status', 'personal_email_id')->first();
            if ($query) {
                $data = $query;
                $message = "Employee fetched Successfully";
                $status = true;
    ...
    {
        try {
            $query = Sender::where('employee_id', $emp_id)->select('*')->first();
            if ($query) {
                $data = $query;
                $message = "Employee fetched Successfully";
                $status = true;
